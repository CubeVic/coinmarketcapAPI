...
UPDATEMANY = """UPDATE prices SET cmc_rank=?,max_supply=?,circulating_supply=?,total_supply=?,last_updated=?,
			...
			if last_updated_from_db != last_updated_from_file:
				...
				for i in range(len(data_dump)):
					...
					r = (data['cmc_rank'], data['max_supply'], data['circulating_supply'], data['total_supply'],
						 data['last_updated'],
						 data['quote']['USD']['price'], data['quote']['USD']['percent_change_1h'],
